...
class MnistMlp(object):
    ...
    def model(self):
        ...
        input = tf.cast(next_op[0]/255, dtype=tf.float32)
        labels = tf.one_hot(indices=tf.reshape(next_op[1], shape=[-1]), depth=100)

        conv_output = self._conv_layer(inputs=input, out_filters=128, name_scope="conv_layer")
        conv_output = self._conv_layer(inputs=conv_output, out_filters=32, name_scope="conv_layer")
        ...
        hidden2_output = self._dense_layer(hidden1_output, num_units=256, dropout_rate=0.2, name_scope='hidden_layer2')
        logits = self._dense_layer(hidden2_output, num_units=100, dropout_rate=None, name_scope='logits')
